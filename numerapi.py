...
class NumerAPI(object):
    ...
    def download_current_dataset(self, dest_path, conf, unzip=True):
    ...
    def get_competition(self, round_id=None):
        ...
        # otherwise set up the request with the specified round ID
            ...
            comp_req_url = self.__get_url(
                "competitions_by_id", query_params=jsonq)
    ...
    def get_submission_for_round(self, username=None, round_id=None):
        ...
        for user in competition["leaderboard"]:
            if user["username"] == username:
                ...
                career_usd = np.float(
                    user["earnings"]["career"]["usd"].replace(",", ""))
                career_nmr = np.float(
                    user["earnings"]["career"]["nmr"].replace(",", ""))
        ...
        self.logger.warning(
            "user \"{0}\" is not on leaderboard".format(username))
        return ()
